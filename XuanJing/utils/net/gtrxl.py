...
class StableTransformerXL(torch.nn.Module):
    ...
    def init_memory(self, shape, device=torch.device("cpu")):
        return [
            # torch.empty(0, dtype=torch.float).to(device)
            torch.zeros(shape, dtype=torch.float).to(device)
    ...
    def forward(self, inputs, memory=None):
        ...
        if memory is None:
            memory = self.init_memory(inputs.size(), inputs.device)
        ...
        return {"logits": layer_out.transpose(0, 1), "memory": memory}
