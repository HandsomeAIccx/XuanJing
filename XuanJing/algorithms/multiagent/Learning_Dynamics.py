"""
ref: https://github.com/Giovanni3A/LearningDynamics/blob/master/examples/HawksDoves/dynamics_comparison.ipynb
"""
    ...
    def run(self):
        ...
        for i, _ in enumerate(self.strategies):
            ...
            if self.ed == 'replicator':
                ...
                xdot = (np.exp(Fi / self.eta) / sum(
                    [np.exp(self.F[j] / self.eta) for j in range(len(self.strategies))])) - xi

            elif self.ed == 'smith':
                d1 = sum([self.state[j] * (self.F[i] - self.F[j]) for j in range(len(self.strategies)) if
                          self.F[j] < self.F[i]])
                ...
                xdot = xi * (Fi - Fhat + sum(
                    [self.state[j] * np.log(self.state[j] / self.state[i]) for j in range(len(self.strategies))]))
...
class Evaluator:
    ...
    def __init__(self, payoff_matrix,
                 ...
                 strategies = ['A', 'B', 'C'],
                 T = int(T),
                 dynamics=['replicator', 'logit', 'smith', 'qlearn', 'pgrad', 'ppo']):
...
if __name__ == "__main__":
    e = Evaluator('DHM', strategies=['D', 'H', 'M'])
    e.evaluate()
