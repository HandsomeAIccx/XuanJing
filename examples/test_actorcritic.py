# -*- coding: utf-8 -*-
# @Time    : 2022/9/24 10:07 上午
# @Author  : Zhiqiang He
# @Email   : tinyzqh@163.com
# @File    : test_actorcritic.py
# @Software: PyCharm
...
if __name__ == "__main__":
    ac_args = ac_args()
    np.random.seed(ac_args.seed)
    torch.manual_seed(ac_args.seed)

    envs = VectorEnv([lambda: gym.make('CartPole-v0') for _ in range(ac_args.env_num)])

    actor_net = MLP(
        input_dim=int(np.prod(envs.observation_space.shape)),
        output_dim=int(np.prod(envs.action_space.n)),
        hidden_sizes=ac_args.actor_net,
    )

    actor = SoftmaxActor(actor_net, envs, ac_args)
    optim = torch.optim.Adam(actor_net.parameters(), lr=ac_args.lr)
