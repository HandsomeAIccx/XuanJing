...
def build_train_bc(args):
    ...
    actor = SoftmaxActor(gail_actor_net, env, args)
    optimizer = torch.optim.Adam(gail_actor_net.parameters(), lr=args.lr)
    ...
    with tqdm(total=bc_args.n_iterations, desc="进度条") as pbar:
        for i in range(bc_args.n_iterations):
            ...
            current_return = np.sum(post_data.get_value('reward'))
